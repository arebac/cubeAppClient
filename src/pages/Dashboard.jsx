...
const Dashboard = () => {
  ...
  }, [isAuthLoading, dashboardUserData, userRole, showScheduleView, cardMinHeight]);
  ...
  }, [dashboardUserData, scheduleData, showScheduleView, expandedClassId, userRole]);

  // --- Data Fetching Logic (Simplified for Coach Today's View) ---
  const fetchScheduleData = useCallback(async (isExplicitRefresh = false) => {
    ...
    if (isExplicitRefresh) { setActionError(null); setActionSuccessMessage(null); }
    ...
    try {
      ...
      if (!res.ok) { let e=`Failed: ${res.status}`; try{const d=await res.json();e=d.message||e;}catch(_){} throw new Error(e); }
  ...
  const handleToggleScheduleView = () => {
    ...
    if (currentIsShowingSchedule && !newShowScheduleView) { // Flipping from BACK to FRONT
  ...
  const handleDropClass = async (classId) => {
    if(!dashboardUserData||!(dashboardUserData.id||dashboardUserData._id)){setActionError("Auth err");return}if(!classId){setActionError("Invalid class");return}const tkn=localStorage.getItem("token");if(!tkn){setActionError("No token");return}setUserDropLoading(classId);setActionError(null);setActionSuccessMessage(null);try{const r=await fetch("http://localhost:5001/api/user/drop-reservation",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${tkn}`},body:JSON.stringify({classId})});const res=await r.json();if(!r.ok)throw new Error(res.message||`Fail ${r.status}`);setActionSuccessMessage(res.message||"Dropped!");if(typeof fetchAndUpdateUser==="function")await fetchAndUpdateUser();await fetchScheduleData(true);}catch(e){setActionError(e.message||"Could not drop");}finally{setUserDropLoading(null);}
  };
  ...
  const getNextDateForDay = (dayOfWeek) => {if(dayOfWeek===undefined||dayOfWeek===null)return null;const t=new Date(),cD=getDay(t);try{const d=Number(dayOfWeek);if(isNaN(d)||d<0||d>6)throw new Error(`Invalid day: ${dayOfWeek}`);return cD===d?t:nextDay(t,d);}catch(e){console.error("Date calc error:",e,"Input:",dayOfWeek);return null;}};

  // --- Render Guards ---
  if (isAuthLoading && !user) {
    return <div className={styles.container}><p className={styles.loadingText}>Loading Session...</p></div>;
  }
  if (!dashboardUserData) {
    return <div className={styles.container}><p className={styles.errorText}>Please log in to view dashboard.</p></div>;
  }
  ...
  return (
    <div className={styles.container} ref={pageContainerRef}> {/* Assign ref here if .container scrolls */}
      <div
        ref={cardRef}
        className={`${styles.card} ${showScheduleView ? styles.cardFlipped : ""}`}
        style={{ minHeight: typeof cardMinHeight === 'number' ? `${cardMinHeight}px` : cardMinHeight }}
      >
        {/* ===== FRONT FACE ===== */}
        <div ref={frontFaceRef} className={`${styles.cardFace} ${styles.cardFaceFront}`}>
          ...
          <div className={styles.actions}>
            <button type="button" className={styles.btn} onClick={handleToggleScheduleView} disabled={showScheduleView && scheduleLoading && scheduleData.length === 0}>
        ...
        <div ref={backFaceRef} className={`${styles.cardFace} ${styles.cardFaceBack}`}>
          <div className={styles.reservationsContainer}>
            ...
            {scheduleLoading && scheduleData.length === 0 && <p className={styles.loadingText}>Loading schedule...</p>}
            {scheduleError && <p className={styles.errorText}>Error: {scheduleError}</p>}

            {(!scheduleError && (scheduleData.length > 0 || (scheduleLoading && scheduleData.length > 0))) && (
                    ...
                    if (userRole === "coach") {
                      ...
                      return (
                        <div key={item._id} className={`${styles.reservationCard} ${styles.coachCard}`}>
                          ...
                          <button type="button" className={styles.btnToggleAttendees} onClick={()=>handleToggleCoachAttendees(item._id)}>
                          ...
                          {isCurrentlyExpanded && (<div className={styles.attendeeList}>
                              {(item.attendees && item.attendees.length > 0) ? (
                                <ul>{item.attendees.map(att=>(<li key={att._id||att.id||att.email}>{att.name} {att.email && <span className={styles.attendeeEmail}>({att.email})</span>}</li>))}</ul>
                      ...
                      return (
                        <div key={item._id} className={styles.reservationCard}>
                          <h3>{nextOccurrenceDate?format(nextOccurrenceDate,'EEEE, MMM d'):(item.dayOfWeek!==undefined?`Day ${item.dayOfWeek}`:'Date Error')}</h3>
                          <p>{item.title}<br/>{timeString}</p>
                          <button type="button" className={styles.btnDrop} onClick={()=>handleDropClass(item._id)} disabled={isLoadingUserAction}>{isLoadingUserAction?'Dropping...':'Drop'}</button>
                        </div>
                      );
                    }
                  })}
